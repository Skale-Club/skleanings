# Requirements Archive: v8.0 Multi-Tenant Architecture
...
- [x] **MT-06**: `DatabaseStorage` refatorado para `DatabaseStorage.forTenant(tenantId: number)` — retorna instância com todas as queries filtradas por `WHERE tenant_id = tenantId`
...
- [x] **MT-08**: Storage singleton existente (`export const storage`) mantido como `storage.forTenant(1)` para compatibilidade durante a transição
