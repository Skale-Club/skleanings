...
- [ ] **SF-06**: `/accept-invite` public page — fetches `GET /api/auth/validate-invite?token=...` on load; shows company name + pre-filled email + Name + Password + Confirm Password form; submits `POST /api/auth/accept-invite`; on success redirects to the tenant's `/admin` URL; on 410 shows "Invitation expired or already used" error
